...
if(null !== filter_input(INPUT_POST, 'cantus_create_submit')) {
    ...
    if($form_valid) {
        ...
        $sql = "insert into cantus (beginning, name, shortname, cycle, tone, month, day, position, title, description, keywords) values ('$beginning', '$name', '$shortname', $cycle, $tone, $month, $day, $position, '$title', '$description', '$keywords')";
                            ...
                            <div class="col-12 col-md-3">
                                <div class="form-group">
                                    ...
                                    <select id="cycle" name="cycle" class="form-control<?=$cycle_valid ?>" title="Круг богослужений" required="required">
                                        ...
                                        <option value="<?=$cycle ?>"<?= filter_input(INPUT_POST, 'cycle') == $cycle ? " selected='selected'" : "" ?>><?=CYCLE_NAMES[$cycle] ?></option>
                                        <?php endforeach; ?>
                                    </select>
